...
//         include: [{ model: Temperament, attributes: ["nombre"] }]
//       }),
//       axios.get("https://api.thedogapi.com/v1/breeds?api_key=live_NvQ2yVkEFDjFB6jyaZo9qQxf3QYSYWolsuz4DcezHxujY7FtSTooi0ZZuLFH2Lxk")
//     ])
//       .then(([dogsDBResponse, dogsAPIResponse]) => {
//         let dogsDB = dogsDBResponse.map(dog => {
//           dog = { ...dog.toJSON() };
//           let temperaments = dog.Temperaments.map(temp => temp.nombre);
//           temperaments = temperaments.join(", ");
//           return { ...dog, temperament: temperaments };
//         });

//         const dogsAPI = dogsAPIResponse.data;

//         const filtro_dogs = dogsAPI.map(dog => {
...
//           const filtroName = alldogs.filter(dog => dog.name.includes(name));
...
//             let dogs = dogsAPI.data.filter(dog => dog.id === parseInt(id))
